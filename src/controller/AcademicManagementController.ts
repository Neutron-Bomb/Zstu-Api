...
async function getAcademicManagement(req: Request) {
    ...
    if (cookie && cookie.expire <= (new Date()).getTime() || !cookie) {
        cookieModel.findOneAndUpdate( { $and: [ { studentId: studentId }, { permission: permission }, { password: password} ]}, { cookieJar: am.getCookieJar()?.toJSON(), expire: (new Date()).getTime() + 1800 * 1000 }, { upsert: true, new: true }).catch(err => console.log(err))
    }
    return am
}
